...
/** The Google text-to-speech v1/voices API endpoint. */
export const voices = {
  /**
  ...
  async list(languageCode?: string): Promise<VoiceInfo[]> {
    ...
    try {
      ...
      const response = await axios.get<VoicesListResponse>(`https://texttospeech.googleapis.com/v1/voices?key=${integrations[0].definition.accountId}${langParam}`, {
        headers: {
